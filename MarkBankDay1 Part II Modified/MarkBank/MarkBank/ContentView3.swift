...
struct ContentView3: View {
    ...
    var body: some View {
        ...
        VStack {
            ...
            HStack{
                ...
                TabView(selection: $currentStep){
                    ForEach(0..<onboardSet.count, id:\.self) { item in
                        
                        VStack{
                            Image(onboardSet[item].image)
                                .resizable()
                                .clipShape(Circle())
                                .frame(width: 250, height: 250)
                               
                            
                            Text(onboardSet[item].title)
                                .font(.system(size: 28, weight: .bold, design: .rounded))
                              
                            
                            Text(onboardSet[item].description)
            ...
            HStack{
                ForEach(0..<onboardSet.count){ it in
                    if it == currentStep{
                        Rectangle()
        ...
        Button(action: {
            if self.currentStep < self.onboardSet.count - 1 {
                self.currentStep += 1
            }
        }) {
            Text(currentStep < self.onBoardSet.count - 1 ? "Next" : "Get Started")
                .padding(16)
